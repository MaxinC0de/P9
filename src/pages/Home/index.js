...
// I- Problèmes
  // 1- last est undefined 
  // 2- les ancres du menu ne sont pas fonctionnelles
  // 3- les props de eventCard sont undefined

// II- Solutions 
  // 1- on appelle data et on définit last à partir de celui-ci grâce à la fonction sort
  // 2- ajout d'id aux trois différents headings du header
  // 3- écrire chaque prop suivant l'exemple suivant imageSrc={last?.cover || ""}

// III- Explications 
  // 1- on appelle ici un prop "last" qui n'est pas défini dans useData()
  // 3- dans le fichier jsx du component eventCard l'on remarque que pour les props imageSrc, title et date de EventCard, qu'un type string est requis
    // donc on ajoute || (ou) "" afin d'avoir une string vide si jamais les props sont undefined, ainsi on a une string dans tous les cas

// IV- Fonctionnement 
  // 1- la syntaxe (const {data} = useData()) utilise la destructuration des props
      ...
      <div className="FormContainer" id="contact">
        ...
        >
          {({ isOpened, setIsOpened }) => (
            <Form
              onSuccess={() => setIsOpened(!isOpened)}
              onError={() => null}
            />
          )}
        </Modal>
